...
<div class="all">
    ...
    <div class="main">
        ...
        <div class="context">
            ...
            <div id="beout1" class="beout1">
                <div class="left">
                    <div class="left_top">
                        <span class="left_top_span">共 ${chapterList.size()} 章</span>
                    </div>
                    <div class="draft_float" style="height:93%;overflow:auto;">
                        <c:forEach var="draftList1" items="${chapterList }">
                            <a href="${pageContext.request.contextPath}/Writer/GetDraftUpload.action?chpaternum=${draftList1.chapterNum}&bookId=${book.bookId}">
                                <div class="draft_index">
                                    <span class="draft_title">${draftList1.chapterName}</span>
                                    <span class="draft_num">${draftList1.chapterWordNum}字</span>
                                    <span class="draft_time"><fmt:formatDate value="${draftList1.chapterRefreshTime}" type="both" pattern="yyyy/MM/dd HH:mm:ss"/></span>
                                </div>
                            </a>
                        </c:forEach>
                    </div>
                    ...
                    <div class="right">
                        <div class="right_top">
                            ...
                            <select class="select1" name="select1" >
                                <option value="${chapter.chapterStatu}">
                                    <c:if test="${chapter.chapterStatu == false}">
                                        公共章节
                                    </c:if>
                                    <c:if test="${chapter.chapterStatu == true}">
                                        收费章节
                                    </c:if>
                                </option>
                            ...
                            <span id="chapter_num" class="chapter_num" style="left: 650px;top: 20px">${chapter.chapterWordNum}</span>
                            <input name="wordnum" id="wordnum" type="hidden" value="${chapter.chapterWordNum}"/>


                            <button id="chapter_save" type="submit" class="chapter_save" style="left: 700px">修改</button>

                        </div>
                        <input id="title" type="text" name="title" class="chapter_title" value="${chapter.chapterName}"></input>
...
<script type="text/javascript">
    document.getElementById("chapter_save").onclick = function () {
        alert("提交审核");
        document.chapter.action = "${pageContext.request.contextPath}/Writer/UpdateChapter.action?chapterNum=${chapterNum}";
