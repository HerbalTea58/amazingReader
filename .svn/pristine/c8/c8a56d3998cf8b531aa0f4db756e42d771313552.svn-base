...
<div class="all">
    ...
    <div id="changePass" class="popBox">
        ...
        <button id="close1" type="button" class="submit1">取消</button>
    </div>
    <div class="main">
        <div class="All_label">
        ...
        <div class="context">
            ...
            <div id="allincome1" class="allincome">
                ...
                <div class="detail1">
                    ...
                    <span class="detail_span4">总收入:${AllIncome }</span>
                    <span class="detail_span2">账户余额:</span><span id="detail_span5" class="detail_span5">${LeftIncome/100.00}</span>
                    <button id="getmoney" class="detail_span3" type="button">提现</button>
                    <div id="pop_getMoeny" class="popBox" style="height: 200px">
                        <span class="popBoxHandle">提现</span>
                        <span class="getmoney1">提现金额:</span>
                        <input id="money" class="getmoney2"/>
                        <button type="button" id="getmoney3" class="getmoney3">提现</button>
                        <button type="button" id="close" class="getmoney4">取消</button>
                    </div>
                            ...
                            <c:if test="${not empty AllOrder}">
                                <c:forEach var="order" items="${AllOrder }">
                                    <tr name="allorder" style="align-text: center;display: none">
                                        <td style="width:100px;height: 40px;line-height: 40px; text-align: center;border: #dddddd 1px solid;">${order.readerOrderId}</td>
                                        <td style="width:150px;height: 40px;line-height: 40px; text-align: center;border: #dddddd 1px solid;">${order.bookName}</td>
                                        <td style="width:100px; height: 40px;line-height: 40px; text-align: center;border: #dddddd 1px solid;">${order.orderTypeName}</td>
                                        <td style="width:180px; height: 40px;line-height: 40px; text-align: center;border: #dddddd 1px solid;">${order.readerNeckName}</td>
                                        <td style="width:100px; height: 40px;line-height: 40px; text-align: center;border: #dddddd 1px solid;">${order.readerOrderPrice}</td>
                                        <td style="width:200px;height: 40px;line-height: 40px;text-align: center;border: #dddddd 1px solid;"><fmt:formatDate value="${order.readerOrderTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                    </tr>
                                </c:forEach>
...
<script type="text/javascript">
    ...
    EventUtil.addEvent(EventUtil._$('getmoney'), 'click', function () {
        p.show({
            /*            x:300,
                        y:300*/
        });
    ...
    EventUtil.addEvent(EventUtil._$('getmoney3'),'click', function() {
        //自定义点击取消按钮之后得操作
        Upmoney();
        //隐藏弹出框
        p.hide();
...
<script type="text/javascript">
    ...
    EventUtil.addEvent(EventUtil._$('close1'),'click', function() {
        //自定义点击取消按钮之后得操作
        //隐藏弹出框
        p1.hide();
    });
